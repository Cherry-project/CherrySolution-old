...
//si on trouve title
    if (strpos($lignewebpage, '<item><title>')!==false){
		...
		$lignewebpage=$coup[0];
		if($lignewebpage!==""){
			$pos = strpos($lignewebpage, "-");
			if ($pos !== false) {
				echo "LIGNEEEE : ";
				$lignetest=explode("-",$lignewebpage);
			    $lignewebpage = $lignetest[0];
			}
		}
		
		/*$test=explode('"',$lignewebpage);
		print_r($test);*/
		//$lignewebpage = str_replace('"', ' ', $lignewebpage);
		/*
		$pos = strpos($lignewebpage, "'");
		if ($pos === true) {
		...
		$reponsebot=$coup2[0].' '.$lignewebpage.' '.$coup2[1];
