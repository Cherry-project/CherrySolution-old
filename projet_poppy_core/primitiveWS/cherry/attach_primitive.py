...
from chatterbot.primitiveRennes import testRennes
...
from chatterbot.chatt import Chatt
...
def attach_primitives(robot, isCamera=True):
    ...
    """
    #os.chdir('./moves')
    print "Importation des moves"
    currentPath = os.path.dirname(os.path.realpath(__file__))
    print "Looking into " + currentPath + "/moves/*.move"
    for file in glob.glob(currentPath + "/moves/*.move"):
        move_name = basename(os.path.splitext(file)[0])
        print(move_name)
        robot.attach_primitive(PlayMove(robot,movement=move_name),move_name)
    print "Fin de l'importation des moves"
    #os.chdir('../')
    ...
    robot.attach_primitive(SayFR(robot),"say_fr")
    robot.attach_primitive(SayEN(robot),"say_en")
    robot.attach_primitive(SayES(robot),"say_es")
    robot.attach_primitive(SayDE(robot),"say_de")
    ...
    robot.attach_primitive(Chatt(robot),"chatt")

    # test Watson API, Rennes
    robot.attach_primitive(testRennes(robot),"test_rennes")
