...
class Voice(object):
    ...
    def go(cls,text,lang):
        ...
        tts = gTTS(HTMLParser().unescape(text).encode('utf-8').decode("utf8"), lang=lang)
        #tts = gTTS(HTMLParser().unescape(text).encode('utf-16'), lang=lang)
        currentPath = os.path.dirname(os.path.realpath(__file__))
        tts.save(currentPath + "/../tmp/temp.mp3")
        
        pygame.mixer.init()
        pygame.mixer.music.load(currentPath + "/../tmp/temp.mp3")
        pygame.mixer.music.play()

        while pygame.mixer.music.get_busy():
            pass
        # load the configuration file that give the server addr and port for requests 
        json_data = open(currentPath + '/../config/conf.json')
