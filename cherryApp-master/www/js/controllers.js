...
.controller('MovesCtrl', function($scope, $http,$rootScope,$ionicPopup) {
    ...
    $scope.run_move = function(index) {
        ...
        console.log($rootScope.primitives[index]);
...
.controller('ChoregraphyCtrl', function($scope, $ionicPopup, $http, $timeout,$rootScope) {
    // Introduction text
    var text= 'Poppy danse sur une musique de ton choix '+$rootScope.username+'. Deplace les icones pour choisir tes mouvements';
    $rootScope.httpSpeech(text);
    ...
    $scope.playAudio = function() {
        if($scope.playingButtonImg=="img/play_red.png"){
            ...
            $http.get($rootScope.cherryUrl+"app/ismoving?id=sogeti").success(function(response){
            ...
            // Stop the robot
            $http({
                url: $rootScope.cherryUrl+"app/stop?id=sogeti",
                method: 'GET',
                data: {"id":'Cherry'},
                headers: {
                    'Content-Type': 'application/json' // Note the appropriate header
