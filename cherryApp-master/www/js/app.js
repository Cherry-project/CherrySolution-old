...
.run(function($ionicPlatform,$rootScope, $http,$ionicPopup) {
    ...
    $rootScope.cherryUrl="http://192.168.1.100:8080/";


    //Get the list of primitives from the server

    $http.get($rootScope.cherryUrl+"app/primitives"+"?id=sogeti").success(function(response)
    {
    ...
    $rootScope.run_move = function(moveName) {
        console.log('run move : ' + moveName)
        $http.get($rootScope.cherryUrl+"app/behave?id=sogeti&name=" + moveName);
    };
