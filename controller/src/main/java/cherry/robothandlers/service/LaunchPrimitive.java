...
public class LaunchPrimitive {
	...
	public static void stopPrimitive(String url) {
		...
		try {
			...
			HttpConnection.sendGet(
					url + "/primitive/" + jsonObject.getJSONArray("running_primitives").getString(0) + "/stop.json");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public static void startSpeakPrimitive(String txtString) {
		 ...
		 * if(txtString.indexOf("(dontwait)") != -1) { dontWait = true; txtString =
		 * txtString.substring(txtString.indexOf(")")+1);
		 * 
		 * }
		 */
		...
		try {
			...
			if (dontPlay != true) {
				...
				HttpConnection.sendPost(
						currentUrlToRobot + "/primitive/" + primitive + "/property/" + property + "/value.json", str);
			}
	...
	public static String getListenStateParameter() {
		...
		try {
			// Local IP for the moment -> to be changed
			listen_state = HttpConnection
					.sendGet("http://127.0.0.2:8888" + "/primitive/" + primitive + "/property/" + property);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public static void setListenStateParameter(String txtString) {
		...
		try {
			// Local IP for the moment -> to be changed
			HttpConnection.sendPost(
					"http://127.0.0.2:8888" + "/primitive/" + primitive + "/property/" + property + "/value.json", str);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public static void setSpeakLanguage(String langString) {
		...
		try {
			HttpConnection.sendPost(
					SetupController.urlToRobot + "/primitive/" + primitive + "/property/" + property + "/value.json",
					str);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	// Generic method to set the property "property" of the primitive "primitive"
	 ...
	 * 
	 * 
	 * String primitive = "play_movement"; String property = "movement";
	 * 
	 * String str = string ;
	 * 
	 * // Start Primitive
	 * 
	 * try { HttpConnection.sendPost("http://127.0.0.2:8888" + "/primitive/" +
	 * primitive + "/property/" + property +"/value.json", str); } catch (Exception
	 ...
	 * // Start Primitive
	 * 
	 * try { //logger.info("Set Language to : " + langString);
	 * HttpConnection.sendGet("http://127.0.0.2:8888" + "/primitive/" + primitive +
	 * "/start.json"); } catch (Exception e) { // TODO Auto-generated catch block
	 * e.printStackTrace(); }
	 * 
	 * }
	 */
